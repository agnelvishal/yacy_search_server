...
if which curl > /dev/null; then
  while getopts "ys" opt; do
    ...
    y)
      shift;
      curl -sSf "http://$1//api/webstructure.xml?about=$2" | awk '/^<link>/{ gsub("<link>","" );gsub("<\/link>","" ); print $0 }'
      ;;
    s)
      shift;
  ...
  while getopts "ys" opt; do
    ...
    y)
      shift;
      wget -q -O - "http://$1/api/webstructure.xml?about=$2" | awk '/^<link>/{ gsub("<link>","" );gsub("<\/link>","" ); print $0 }'
      ;;
    s)
      shift;
